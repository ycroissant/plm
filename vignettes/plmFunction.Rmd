...
wide range of error component models. Those models can be writen as
follow :
...
the data. The *gls* model requires more explanations. In most of the
...
droping the expecting value operator. Some degree of freedom
...
- `method` is an alternative to the `models` argument ; it is one of :
    ...
    * `nerlove` which is a specific method which doesn't fit to the
      methodolgy described aboved (@NERLO:71).
...
The estimated variance can be extract using the `ercomp` function. For
...
to `TRUE`. @BALT:13 p. 44-46, presents Eviews output for the Grunfeld
data set.
...
screenreg(list("Wallace-Hussein" = Grw, "Swamy-Arora" = Grs, "Amemiya" = Gra), digits = 5)
```

The estimated variance of the time component of the variance is
negative for `whalus` and `swar`, it is set to 0.
...
screenreg(list("Wallace-Hussein" = Prw, "Swamy-Arora" = Prs, "Amemiya" = Pra), digits = 5)
```
...
- there are no obvious denominators for the quadratic forms of the
  ...
  equates it to the actual quadratic forms. Detailed formula are
  omited here, they depend on the preliminary estimator.
...
presents the Stata output as @BALT:09 p.211 presents the Eviews
output. The Eviews Wallace Hussain estimator is reported in @BALT:09,
p. 210.
...
screenreg(list(Eviews = HedEviews, Stata = HedStata, "Walace-Hussein" = HedEviewsWH), 
          digits = 5, single.row = TRUE)
```

The difference is due to the fact that Stata use a between regression
on $N$ observations as Eviews a between regression on $\sum_n T_n$
observations, which are not the same on unbalanced panels. Note the
use of between with or without the B is capitalized in the
random.models argument.
...
screenreg(list(within = crwth, EC2SLS = crbalt, G2SLS = crbalt), 
...
screenreg(list("Swamy-Arora" = swar, "Walhus-Hussein" = walhus, "Amemiya" = amem), digits = 5)
```
