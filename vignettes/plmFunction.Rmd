...
To illustrate the use of `plm`, we use examples reproduced in @BALT:2013.
...
@BALT:2013, p. 27, presents the Stata estimation of the Swamy-Arora
...
to `"twoways"`. @BALT:2013 p. 44-46, presents EViews' output for the Grunfeld
data set.
...
@BALT:2013 and @BALT:09 present results of the estimation of the
@SWAM:AROR:72 model with the `Hedonic` data set. @BALT:2013, p. 174,
...
defined (in @BALT:CHA:94, p. 73). A more detailed analysis of Stata's
...
- `"baltagi"`, from @BALT:81, the instruments of the second part are
  ...
  transformation,
- `"am"`, from @AMEM:MACUR:86, the within transformation of the variables
  of the second parts for each period are also included as
  instruments,
- `"bmc"`, from @BREU:MIZO:SCHMI:89, the within transformation of the
...
from @BALT:2005, p. 120/ @BALT:2013, p. 137.
...
The following example is from @BALT:2005, p. 130 and @BALT:2013, p. 146.
