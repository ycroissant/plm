...
taxonomy\footnote{Comprehensive treatments are to be found in many
  econometrics textbooks, e.g. \citet{BALT:01, BALT:2013} or \citet{WOOL:02, WOOL:2010}:
  the reader is referred to these, especially to the first 9 chapters
  of \citet{BALT:01, BALT:2013}.}.  Section~\ref{sec:software} discusses the
...
\citep{BALT:01}, p.174/\citet{BALT:2013}, p.197.
...
% TODO:  "am", "bmc"/"bms" missing here?
\end{itemize}
 as
illustrated on the following example from \cite{BALT:01}, p.120/\citet{BALT:2013}, p.133/136 ("EC2SLS").
...
cr <- plm(log(crmrte) ~ log(prbarr) + log(polpc) + log(prbconv) +
         ...
         west + central + urban + factor(year) | . - log(prbarr) -log(polpc) + 
...
\cite{BALT:01}, p.130/\citet{BALT:2013}, p.146.
...
results obtained by running the above Augmented Dickey-Fuller
regression. 
...
\citep[see][chap~5]{BALT:01, BALT:2013}, which is a very common specification in
