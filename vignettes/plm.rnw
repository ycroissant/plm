...
panel data sets often happen to be {\em unbalanced} (i.e., they have a
...
The four data sets used are \code{EmplUK} which was used by
...
the first two columns, as in all the example data sets but \code{Wages}.
...
Most of the features of \code{plm} are implemented for unbalanced panel models
with one notable exception:

\begin{itemize}
\item there is no random two-ways effect model for \code{random.method = "nerlove"}.
\end{itemize}
...
model cannot be estimated by \textsc{ols} anymore, therefore the testing
...
model objects only for T ``sufficiently large''\footnote{Baltagi and
  ...
  effects residuals is documented in \citet{BHAR:FRAN:NARE:82}, a
  (modified) version for unbalanced and/or non-consecutive panels is
  implemented in \code{pbnftest} as is Baltagi-Wu's LBI statistic 
  (for both see \citet{BALT:WU:99}).}
