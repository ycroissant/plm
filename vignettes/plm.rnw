...
model objects only for T ``sufficiently large''\footnote{Baltagi and
  ...
  modified version for unbalanced and/or non-consecutive panels in 
  \citet{BALT:WU:99}, the statistic and Baltagi-Wu's LBI statistic 
  are implemented in \code{pbnftest}}.
