...
- extract from the `model.frame` the response (with `model.response`),
...
second position. This unusual order of the arguments unables to use a
...
needs actualy one argument and not two and we therefore wrote a
...
- in the common case when a lot of covariates are instruments, these
  ...
  formula,
- the endogenous variables clearly appear as they are preeceed by a
  `-` sign in the second rhs part of the formula.

The formula is coerced to a `Formula`, using the `Formula`
package. `model.matrix.pdata.frame` then internaly calls the
...
head(X1, 3) ; head(W1, 3)
```
For the second (and prefered formulation), the `dot` argument should
...
`dot = "previous"`. See the difference between :
...
the `.` is understanden by default as "everything".

In `plm`, the `dot` argument is internaly set to
`previous` so that the end-user doesn't have to wory about these
subtilties. 
