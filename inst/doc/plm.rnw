...
the data. Specific methods for \texttt{pserie} objects are therefore
have been rewritten in order to handle correctly panel data. Further
functions called \code{Between}, \code{between} and \code{within} are
also provided to compute the between and the within
...
There are two circumstances where standard \code{formula} are not very
usefull : formulas for dynamic models and formulas for instrument
variable estimation. Two deal with the first situation, we provide a
class called \code{dynformula}, for the second situation, we use the
\code{Formula} package.

\subsubsection{Formula for dynamic models}

Using the \code{lag} and \code{diff} functions to write a formula can
be very cumbersome for dynamic models. A \code{dynformula} function is
provided to ease the writing of such formulas. Suppose for example
than one wants to estimate a model where employment depends on its own
first two lags and on the second and third lags of wages and capital,
and capital being differenced. The formula is then:
...
<<>>=
dynformula(log(emp)~log(wage)+log(capital), lag=list(emp=2,c(2,3)),diff=c(FALSE,capital=TRUE))
@ 

The arguments \code{lag} and \code{diff} are lists/vectors which can
be:

\begin{itemize}
\item unnamed, in this case, the length of the list/vector must equal the
  number of variables,
...
The \code{diff} vector is logical. The elements of \code{lag} are
either a number or a vector of two numbers: 3 means current value and
first three lags, \code{c(1,2)} means first two lags.

\
\subsubsection{Formula for instrument variable estimation}

Two-part formulas are a very convenient way to describe models
estimated using instrument variables, the first part indicating the
covariates and the second part the instruments. The \code{Formula}
package provides a class which unables to construct multi-part
formula, each part being separated by a pipe sign. \code{plm} provides
a \code{pFormula} object which is a \code{Formula} with specific
methods.
