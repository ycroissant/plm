...
particulary interesting when there are a few external instruments, as
...
with \code{plm} by equating the \code{model} argument to \code{"ht"}. The following example is from \cite{BALT:01} p.130.
