...
mtest(ab.a1, 1, vcov = NULL) # -3.9394   xtabond manual, example 1
mtest(ab.a1, 2, vcov = NULL) # -0.54239  xtabond manual, example 1
mtest(ab.a1, 1, vcov = vcovHC) # -3.600 (reference DPD/Ox abest3.out)         / -3.5996 xtabond manual, example 2
mtest(ab.a1, 2, vcov = vcovHC) # -0.516 (reference A/B and DPD/Ox abest3.out) / -0.51603 xtabond manual, example 2
...
mtest(ab.a2, 1, vcov = vcovHC) # -2.1255  xtabond manual, example 4
mtest(ab.a2, 2, vcov = vcovHC) # -0.35166 xtabond manual, example 4
...
mtest(ab.b, 1, vcov = NULL)   # -2.826 (reference about A/B's m1 in Windmeijer) / DPD/Ox has -2.428 in abest1.out
mtest(ab.b, 2, vcov = NULL)   # -0.327 (reference in A/B and in Windmeijer)     / DPD/Ox has -0.3325 in abest1.out, explanation in footnote 7 on p. 32
