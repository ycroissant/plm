...
#YC pdiff_time <- plm:::pdiff(Grunfeld[ , "value"], index(pGrunfeld)[[2]], effect = "time")

pos_first_id <- which(Grunfeld$firm == 1)
pos_first_time <- which(Grunfeld$year == 1935)

diff_id   <- base::diff(Grunfeld[pos_first_id,   "value"])
#YC diff_time <- base::diff(Grunfeld[pos_first_time, "value"])

#YC if (!isTRUE(all.equal(pdiff_id[pos_first_id[-length(pos_first_id)]],  diff_id))) stop("pdiff on individual not correct")
#YC if (!isTRUE(all.equal(pdiff_time[pos_first_time[-length(pos_first_time)]], diff_time))) stop("pdiff on time not correct")
